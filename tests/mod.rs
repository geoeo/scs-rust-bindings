...
fn c_example() {
    ...
    let mut P_x : [scs_float;3] = [3., -1., 2.]; /* Upper triangular of P only */
    let mut P_i : [scs_int;3] = [0, 0, 1];
    let mut P_p : [scs_int;3] = [0, 1, 3];
    let mut A_x : [scs_float;4] = [-1., 1., 1., 1.];
    let mut A_i : [scs_int;4] = [0, 1, 0, 2];
    let mut A_p : [scs_int;3] = [0, 2, 4];
    let mut b : [f64;3] = [-1., 0.3, -0.5];
    let mut c : [f64;2] = [-1., -1.];
    /* data shapes */
    let n : scs_int = 2; /* number of variables */
    let m : scs_int = 3; /* number of constraints */
    unsafe {
